...
2. Operator sends the start validator index and the list of amounts to the Relayer, then polls the Relayer for the list of validators.
3. Relayer creates pending validators and exposes them via the API to Sidecars.
4. DVT Sidecars poll the Relayer for pending validators that they can sign, create deposit and exit signatures for those validators, and push the signatures back to the Relayer.
