...
Additionally, ensure your Operator Service wallet has the [Validators Manager →](./validators-manager) role assigned in your Vault settings for proper operation.
:::
...
- **Automated execution every 24 hours** by default (configurable via `PARTIAL_WITHDRAWALS_INTERVAL` environment variable)
- Processes validators with **balances exceeding 32 ETH or 1 GNO**, prioritizing those with higher balances first
...
- **Much faster processing** compared to the previous oracle-dependent system
- **Automatic fallback**: If the Operator Service doesn't initiate withdrawals, oracles will automatically execute full withdrawal after 24 hours
...
- **Disable partial withdrawals**: Use `--disable-withdrawals` flag to process all withdrawals via full validator exits only
- **Adjust frequency**: Set `PARTIAL_WITHDRAWALS_INTERVAL` environment variable to change the default 24-hour interval
