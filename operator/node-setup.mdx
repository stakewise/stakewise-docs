...
- Installed the Operator Service. See the [Installation →](installation) section in Installation.
- Generated validator keys. See the [Validator Keys →](validator-keys) section in Validator Keys.
...
It is recommended to start the nodes together with the Operator Service using the `--run-nodes` flag in the `start` command. For more details, see the [Running the Operator Service →](./start-operator) section.
:::
