...
2. **Initiate exits if needed**. If available ETH is insufficient and partial witdrawals are not possible, the Operator Service initiates validator exits to free up the remaining amount.
3. **Oracle enforcement of exits**. If the Operator Service does not initiate partial or full withdrawals, the Oracle network will automatically execute full exits after 24 hours.
