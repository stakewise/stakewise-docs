...
**5. Running Validators** – You need to run a validator client with the keys that have been allocated to your Vault. These validators will be automatically registered and managed by the Operator Service once it is properly configured with your [validator keys →](/operator/validator-keys) and [Validators Manager →](/operator/validators-manager) wallet.
