...
The OsToken system allows users to gain liquidity on their staked assets without exiting the Beacon Chain validator set. Stakers can trade or use OsTokens in DeFi, while the underlying stake continues to earn consensus-layer rewards and MEV, which accumulate in the originating vault.

An OsToken (e.g., `osETH`, `osGNO`) is an over-collateralized ERC-20 token that represents a claim on staked assets in one or more vaults.
When a vault supports OsToken minting, stakers can lock their vault shares as collateral and mint the corresponding OsToken against their position. The amount that can be minted is limited by the vault's collateralization ratio (LTV) - typically 90% for standard pools, and up to 99.99% (`osETH`) or 99.95% (`osGNO`) for DAO-approved vaults meeting strict requirements<sup><a href="#user-content-fn-2" id="user-content-fnref-2">2</a></sup>.

When a user wants to redeem their collateral, they must burn the OsTokens and unlock their vault shares. Because the system enforces over-collateralization, there is always more staked ETH (or GNO) backing the OsTokens in circulation than required to redeem them, protecting against shortfalls.
