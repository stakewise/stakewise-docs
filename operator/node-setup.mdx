...
- Deployed a StakeWise Vault. See the [Deployed Vault](prerequisites#deployed-vault).
- Installed the Operator Service. See the [Installation](installation).
- Generated validator keys. See the [Validator Keys](validator-keys).
- Configured the Validators Manager. See the [Validators Manager](validators-manager).
...
:::custom-info[Recommendation]
It is recommended to start the nodes together with the Operator Service using the `--run-nodes` flag in the `start` command. For more details, see the [Running the Operator Service](./start-operator).
:::
