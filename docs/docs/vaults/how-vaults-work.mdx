...
The Operator Service monitors Vault balances and submits validator registration requests to the StakeWise [Oracle network](../oracles/intro). Each registration requires [approval from 8 of 11 Oracles](../oracles/oracle-duties#validator-registration-approval) before it is submitted on-chain. Deposit data is generated automatically during registration. Once approved, validators enter the Beacon Chain's [activation queue ↗](https://www.validatorqueue.com/) and begin earning rewards when active.
