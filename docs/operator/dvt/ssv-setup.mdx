...
<p style={{fontSize: '1.2em', color: '#6c757d', marginTop: '-10px', marginBottom: '20px'}}>The below is a quick walkthrough on how to setup validators on StakeWise V3 that run on SSV.</p>
...
4. [Setup Operator Service →](/operator/intro)

## Step 1: Register SSV Operator

You can use the [SSV UI ↗](https://app.ssv.network/connect) for that. After registration run SSV Node.

<img src="/img/ssv setup 1.avif" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Click Join As Operator</p>

<img src="/img/ssv setup 2.avif" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Click Register Operator</p>

<img src="/img/ssv setup 3.avif" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Generate Operator Keys and fill operator public key field</p>
...
```json title="encrypted_private_key.json"
{
  "checksum": {
    "function": "sha256",
...
<img src="/img/ssv setup 4.avif" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Set Operator Fee and click Next</p>

<img src="/img/ssv setup 5.avif" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Click Register Operator</p>

<img src="/img/ssv setup 6.avif" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Done</p>

## Step 2: Run SSV Node

For more information, refer to the [SSV Operator Node Documentation ↗](https://docs.ssv.network/operators/operator-node/node-setup/).
...
:::custom-notes[WebSocket Required]
...
:::custom-warning[Important - No Multiple Instances]
...
If you don't want to use the default ports, they can be changed in your `config.yaml` file. Be aware, they must be changed on the container creation command as well (simply changing the host port mappings on the Docker command isn't enough!).
...
## Step 3: Split validator between SSV Operators

### Offline Key Splitting

Offline key splitting is the most secure option, although less convenient, as it requires running a command line tool. For more information, refer to the specific User Guide on how to use the [ssv-keys CLI tool ↗](https://docs.ssv.network/developers/SSV-SDK/examples/generate-and-validate-keyshares).

Before continue download [ssv-keys ↗](https://github.com/bloxapp/ssv-keys/releases).

:::custom-notes[Bulk Operations Support]
The latest SSV Smart Contract updates added support for bulk operations, and the latest release of `ssv-keys` has been made compatible with bulk operations. With version 1.1.0 (and above), it is possible to generate keyshares for multiple keystores in a single operation.
:::

<img src="/img/ssv%20setup%207.avif" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Select your validator cluster and press Add validator button</p>

<img src="/img/ssv%20setup%208.png" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Press Offline button</p>

<img src="/img/ssv%20setup%209.png" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Select Command Line Interface, copy command from 4 step and press next.</p>

### Generate keyshares

Run the `ssv-keys` command with your operator information:

```bash
$ ./ssv-keys-mac --operator-keys=LS0tL.... --operator-ids=66,391,393,442 --owner-address=0xd676090f860......b9f72b1e356 --owner-nonce=11
```

The command will prompt you for keystore information and generate keyshares:
...
<img src="/img/ssv%20setup%2010.png" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Upload /Users/user/ssv-keys/keyshares-1712562491.json file to Enter KeyShared File window.</p>

<img src="/img/ssv%20setup%2011.png" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Once uploaded, if successfully validated, advance to the next screen clicking Next.</p>
...
<img src="/img/ssv setup 12.avif" alt="SSV Setup" />
<p style={{fontSize: '0.9em', textAlign: 'center', color: 'var(--grey)', fontStyle: 'italic', marginTop: '5px'}}>Once uploaded, if successfully validated, advance to the next screen clicking Next.</p>

:::custom-warning[Important - Fee Management and Liquidation Risk]
Please read carefully and understand how fees are managed and the risks of account [liquidation ↗](https://docs.ssv.network/learn/protocol-overview/tokenomics/liquidations/) if your account balance falls below the Threshold Balance.
:::

:::custom-warning[Important - Stop Other Validator Clients]
...
<img src="/img/ssv%20setup%2013.avif" alt="SSV Setup" style={{marginBottom: '20px'}} />

<img src="/img/ssv%20setup%2014.avif" alt="SSV Setup" />

The next screen presents a summary of your validator setup.

<img src="/img/ssv%20setup%2015.avif" alt="SSV Setup" />
...
:::custom-notes[First Time Registration]
