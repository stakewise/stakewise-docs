...
runs specialized software (the [operator ↗](https://github.com/stakewise/v3-operator) service) alongside their Ethereum consensus and execution clients
that monitors deposit levels and handles the technical validator creation process.

When sufficient funds are available, this service submits validator registration requests to the StakeWise [oracle network →](./Oracles.mdx).
...
After the [Pectra ↗](https://ethereum.org/en/roadmap/pectra/) upgrade, [EIP-7251 ↗](https://eips.ethereum.org/EIPS/eip-7251) allows Vaults to merge multiple 32 ETH validators into fewer `0x02` validators with higher effective balances.
...
Partial withdrawals for compound validators (`0x02`) are now supported.
They are significantly faster and more efficient than full validator exits.
