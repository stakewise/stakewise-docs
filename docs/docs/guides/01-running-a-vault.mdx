...
**🚀 Setup difficulty:** Moderate – requires running Ethereum nodes and basic command-line familiarity. Follow the steps below to get started.

### Setup Steps

**1. [Prerequisites→](/operator/prerequisites)** – Ensure your infrastructure is ready with fully synced [execution →](/operator/prerequisites#execution-client) and [consensus →](/operator/prerequisites#consensus-client) clients, plus an [upgraded Vault →](/operator/prerequisites#deployed-vault) (version 5 for Ethereum, version 3 for Gnosis).

**2. [Installation→](/operator/installation)** – Choose from multiple installation methods: download pre-built binaries, use Docker images, or build from source. The setup is straightforward with just a few simple commands.

**3. [Validator Keys→](/operator/validator-keys)** – Generate validator keystores using built-in tools or import existing keys. Validator keystores don't need to be stored locally—you may prefer to manage them externally using [alternative key management →](/operator/alternative-key-management) options like remote signers, HashiCorp Vault, or API mode.

**4. [Validators Manager→](/operator/validators-manager)** – Create an operator wallet and assign it the Validators Manager role through your Vault settings. This wallet handles validator registrations, funding, and withdrawal operations.

**5. [Start Operator Service→](/operator/start-operator)** – Launch the service with your Vault address and node endpoints using your preferred method (binary, Docker, or source). The Operator Service is highly configurable through the use of [optional flags →](/operator/start-operator#optional-flags).
