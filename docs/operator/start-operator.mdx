...
```bash
./operator start-local --vaults=0x83...87Cf --consensus-endpoints=http://localhost:5052 --execution-endpoints=http://localhost:8545
```
...
For alternative key management methods (Remote Signer, HashiCorp Vault, API Mode), see the [Alternative Key Management →](./alternative-key-management/remote-signer) section.
:::
...
<Tabs groupId="installation-types">
  ...
  <TabItem value="docker" label="Docker">
    ...
    --vaults=0x834F27bC8670491b75af512d943f01D5383F87Cf \
    --data-dir=/data \
...
- `--validator-type` - Set to v1 if you want to register 0x01 validators. By default v2 or 0x02 validators are registered
...
- `--claim-fee-splitter` - Claims fees periodically to the addresses of the shareholders. See [Fee Claiming →](./additional-actions/fee-claiming) for details
