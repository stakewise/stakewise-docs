...
osToken is created through **minting** — new osToken issued against ETH or GNO staked as collateral in a Vault. A maximum **Loan-to-Value (LTV)** ratio caps how much can be minted against that stake, keeping every osToken overcollateralized.

The osToken **exchange rate** is the amount of the underlying asset a user receives from StakeWise when redeeming osToken. It rises as staking rewards accrue, so each osToken is worth more ETH or GNO over time.

**Redemptions** convert osToken back to ETH/GNO at this exchange rate. The protocol uses them to close any gap between osToken's market price and its fair value, keeping the two aligned.
...
Each minter's position falls into one of four grades — Healthy, Moderate, Risky, Unhealthy — based on how close LTV is to the liquidation threshold. In **DAO-approved Vaults** these thresholds don't apply.

<Image img={require('./img/grade.png')} alt="osToken position health grades - Healthy, Moderate, Risky, Unhealthy" />

## Exchange Rate

The osToken exchange rate is the amount of the underlying asset (ETH or GNO) a user receives from StakeWise when redeeming osToken. It serves as the protocol-calculated conversion rate between osToken and the underlying staked asset.
