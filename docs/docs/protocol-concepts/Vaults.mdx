...
An OsToken (e.g., `osETH`, `osGNO`) is an over-collateralized ERC-20 token that represents a claim on staked assets in one or more vaults. StakeWise vaults can optionally integrate with the OsToken system to provide liquidity against a staked position.

When enabled, users may lock their vault shares as collateral and mint an OsToken without unstaking. The amount of OsToken that can be minted is determined by parameters set in the [OsTokenVaultController](https://etherscan.io/address/0x2A261e60FB14586B474C208b1B7AC6D0f5000306#code) and [OsTokenConfig](https://etherscan.io/address/0x287d1e2a8de183a8bf8f2b09fa1340fbd766eb59#code) conatrcts. These parameters include:

**Collateralization ratio (LTV)** – how much underlying stake must back each OsToken. Standard pools offer 90% LTV, while DAO-approved vaults can provide up to 99.99% (osETH) or 99.95% (osGNO), subject to strict requirements such as minimum stake size, low fees, above-median performance, and a SWISE bond.

**Fee rates and other risk controls** – such as liquidation thresholds and redemption mechanisms that protect peg stability.

Users unlock collateral by burning the OsToken. While OsTokens provide liquidity, the underlying stake and rewards remain in the originating Vault, continuing to accrue consensus and execution-layer returns.
