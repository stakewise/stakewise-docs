...
- Deployed a StakeWise Vault. See [Create Your Vault →](/operator/create-your-vault).
- Installed the Operator Service. See [Installation →](/operator/installation).
:::
...
:::custom-info[Tip]
You can also start nodes automatically alongside the Operator Service using the `--run-nodes` flag. See [Start Operator →](/operator/start-operator).
:::
