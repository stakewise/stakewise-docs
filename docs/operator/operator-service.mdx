...
3. Obtain the BLS signature for exit messages<sup><a href="#user-content-fn-1" id="user-content-fnref-1" style={{textDecoration: 'none'}}>1</a></sup> using local keystores or remote signer
4. Share the exit signature of the validator with StakeWise Oracles:
    - Using Shamir's secret sharing, splits validator's BLS signature<sup><a href="#user-content-fn-2" id="user-content-fnref-2" style={{textDecoration: 'none'}}>2</a></sup>. The number of shares equals the number of oracles
    - Encrypts exit signatures with the oracles' public keys
    - Sends encrypted exit signatures to all the oracles and receives registration signatures from them
5. Sends transaction to the Vault contract to register the validator.
:::
