...
There are two types of Vault: a [Regular Vault](./create-regular-vault), which runs validators to secure the Ethereum network and earn rewards, and a [Meta Vault](./create-meta-vault), which doesn't run validators itself and instead allocates stake across multiple sub-vaults.
