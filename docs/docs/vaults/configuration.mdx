...
Vaults that use the Smoothing Pool must set every relay to one of the [StakeWise DAO-approved MEV relays](/operator/smoothing-pool-relays#approved-relay-endpoints). This ensures that individual Vaults' contributions to the Smoothing Pool are tracked and the risk of non-contribution is mitigated.
...
Triggers [claims of accumulated Vault fees](/operator/manage-vault/fee-claiming) on behalf of stakeholders.
