...
<Tabs groupId="operator-flows">
  <TabItem value="registration" label="Registration" default>
    ...
    **Required data:**
    ...
    :::custom-notes[Exit Signature Details]
  ...
  <TabItem value="funding" label="Funding">
    ...
    **Process:**
    1. Operator calls `/fund` endpoint
    2. Relayer responds with signatures
    3. Operator executes funding transaction
  ...
  <TabItem value="withdrawals" label="Withdrawals">
    ...
    **Process:**
    1. Operator calls `/withdraw` endpoint
    2. Relayer responds with authorization signature
    3. Operator submits withdrawal transaction
...
The Validators Manager signature uses [EIP-712 ↗](https://eips.ethereum.org/EIPS/eip-712). See [implementation example ↗](https://github.com/stakewise/relayer-example/blob/main/src/validators/validators_manager.py#L33). The signer address must be the Validators Manager configured in Vault settings.
:::
