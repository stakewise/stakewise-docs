...
| DAO-approved   | 99.99% LTV       | 99.975% LTV    | 5M SWISE operator bond |

To qualify for the 99.99% LTV, Vaults must meet strict performance criteria<sup><a href="#fn-1" id="fnref-1">1</a></sup> and post a SWISE operator bond.
...
Redemptions run through the [`OsTokenRedeemer` ↗](https://etherscan.io/address/0xc43A7b16A7a167c0318390Cba16787C11e9e1FD0) contract. Vaults version 1 (mainnet only) retain a separate position-level mechanism that's currently disabled.

### Redeemer Redemption

osToken's market price on DEXes can drift from the protocol exchange rate. Anyone holding osToken can redeem it at the protocol exchange rate by entering the `OsTokenRedeemer`'s exit queue — no whitelist, no privileged role — which is what makes a discount worth arbitraging away. Queue entry is open to all; only the on-chain execution against minters' positions is carried out by the Operator Service.

See [osToken Redemptions](/docs/ostoken/ostoken-redemptions) for the exact mechanics.

### Position-Level Redemption
...
The protocol calculates the **maximum unstakable ETH** as (the examples below use Ethereum's 99.99% cap; substitute 0.99975 on Gnosis Chain):
...
When a user exits a Vault while still holding osToken debt, the Vault may not have liquid ETH immediately available, since validators must go through the Ethereum exit queue. The [`OsTokenVaultEscrow`](../../contracts/networks/) contract holds the osToken position alongside the underlying stake until ETH is freed, then unwinds both in lockstep. This keeps every osToken fully backed throughout the exit process. Positions inside the escrow have their own liquidation parameters, set separately from the Vault-level values above.
