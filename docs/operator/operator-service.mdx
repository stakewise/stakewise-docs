...
<Admonition type="note" icon={<img src={useBaseUrl('/img/safe.png')} alt="Warning" style={{ width: 20, height: 20 }}/>} title="Validator Registration Process">
  <ol style={{ listStyleType: "none", paddingLeft: "0" }}>
    ...
    <li><span style={{ display: "inline-block", width: "3ch", textAlign: "left" }}>3.</span> Obtain BLS signature for exit message using local keystores or remote signer</li>
    ...
    <ul style={{ listStyleType: "disc", paddingLeft: "80px" }}>
      <li>Using Shamir's secret sharing, splits validator's BLS signature. The number of shares equals the number of oracles</li>
...
The Operator Service automatically processes partial ETH/GNO withdrawals from validators.

<Admonition type="info" icon={<img src={useBaseUrl('/img/eth_coin.png')} alt="Warning" style={{ width: 20, height: 20 }}/>} title="Withdrawal Processing">
  Every 24 hours, the operator checks the current exit queue. If the requested amount can be processed via partial withdrawals, it submits them through the execution layer
</Admonition>
...
Converts legacy 0x01 validators to 0x02 compound validators using the `consolidate` command.
...
Enter the network name (mainnet, holesky, gnosis, chiado) [mainnet]: holesky
...
<Admonition type="warning" icon={<img src={useBaseUrl('/img/exclamation_mark.png')} alt="Warning" style={{ width: 20, height: 20 }}/>} title="Fee Claimer Required">
  Set the wallet address connected to the operator as the `Fee Claimer` in the `Roles` tab under the Vault’s Settings.
</Admonition>

Enable automated reward splitting using the `--claim-fee-splitter` flag:
...
<ol style={{ listStyleType: "none", paddingLeft: "0" }}>
  <li><span style={{ display: "inline-block", width: "3ch", textAlign: "left" }}>•</span> **Distribution Interval**: Configurable via `REWARD_SPLITTER_INTERVAL` (default: 24h)</li>
  <li><span style={{ display: "inline-block", width: "3ch", textAlign: "left" }}>•</span> **Minimum Threshold**: Set via `REWARD_SPLITTER_MIN_ASSETS` to avoid gas-inefficient small transactions</li>
</ol>

### Validators Voluntary Exit

Validator exits are handled by [Oracles →](../../docs/protocol-concepts/Oracles), but you can force trigger exits for your validators using:
