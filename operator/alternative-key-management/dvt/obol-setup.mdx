---
title: Obol Setup
description: Set up distributed validators with Obol Charon and StakeWise Vaults — split existing keys or use DKG, solo or as a group, integrated with the DVT Relayer.
---
...
<Image img={require('./img/validator_signature_flow.png')} alt="Validator Signature Flow" />
...
When the keys are generated centrally, the Operator Service can access them and [registers the validators](/docs/vaults/how-vaults-work#validator-registration) itself.

When the keys are generated through a DKG ceremony, no party ever holds the full key, so the Operator Service can't register on its own. Registration instead runs through the StakeWise DVT components (the Sidecars and the DVT Relayer) — see the [registration process](/operator/alternative-key-management/dvt/overview#validator-registration-process).
:::
...
Step 1 produced one folder per node under `.charon/cluster/` (`node0/`, `node1/`, `node2/`), each holding that node's key share, networking key, and cluster lock file. Give each node folder to a different machine:
...
Each operator runs one [DVT Sidecar ↗](https://github.com/stakewise/dvt-operator-sidecar) on the machine running their node. It loads the node's key shares, polls the Relayer for validator data, and submits deposit and exit signature shares back to the Relayer.
...
Obol's node setup (CDVN) ships a local Grafana dashboard. Once your nodes are running, check their health at `http://localhost:3000/d/charonoverview/`. For hosted dashboards and alerting, see [Obol monitoring ↗](https://docs.obol.org/run-a-dv/start/obol-monitoring).
