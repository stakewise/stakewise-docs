...
A maximum possible fee is 100% of rewards.
This fee is deducted from rewards. The Vault’s displayed APY is always net of fees.
Vault Admin can update current fees through the `setFeePercent` function.
The fee can be updated once every 3 days and may only be increased by up to 20% at a time.
If the initial fee is 0%, the first increase is limited to 1%.
