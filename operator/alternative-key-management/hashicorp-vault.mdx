---
title: HashiCorp Vault
description: Load StakeWise validator signing keys from a remote HashiCorp Vault instance. Configure K/V secret paths, tokens, and key storage format.
---

import { CommandSnippet } from '@site/src/components';

# HashiCorp Vault

The Operator supports loading signing keys from a remote [HashiCorp Vault ↗](https://developer.hashicorp.com/vault/docs) instance, avoiding storage of keystores on the filesystem. This approach is best suited for node operators who already have most of StakeWise Operator functionality implemented in their systems, and only need integration for validators registration and withdrawals.

## Prerequisites

Complete the following steps before proceeding:

1. [Installation](/operator/launch-operator-service#installation) completed
2. [Validators Manager](/operator/validators-manager) set
3. Validator keys stored in your [HashiCorp Vault ↗](https://developer.hashicorp.com/vault/docs) instance

## Key Storage Format

The user must provide the HashiCorp Vault instance URL, authentication token, and secret path in the K/V engine.

Internal structure of the secret must resemble the following JSON format:
  ```json
{
  "pubkey1": "privkey1",
  "pubkey2": "privkey2",
  ...
}
```

Public and private signing keys must be stored in hex form, with or without `0x` prefix.

After loading keys from HashiCorp Vault, the operator behaves in the same way as if it had loaded them from keystores, No additional operations are needed to support the integration.

## Start Operator Service

Passing the following options to the `start-hashi-vault` command will enable loading validator signing keys from remote [HashiCorp Vault ↗](https://developer.hashicorp.com/vault/docs).

<CommandSnippet template={`./operator start-hashi-vault \\
  --network={{mainnet}} \\
  --vault={{0x834F27bC8670491b75af512d943f01D5383F87Cf}} \\
  --consensus-endpoints={{https://consensus-node}} \\
  --execution-endpoints={{https://execution-node}} \\
  --hashi-vault-key-path={{keystores1}} \\
  --hashi-vault-key-path={{keystores2}} \\
  --hashi-vault-token={{hvs.abcde}} \\
  --hashi-vault-url={{https://hashi-vault:8200}}`} />

### `start-hashi-vault` flags

| Flag | Description |
|------|-------------|
| `--hashi-vault-url` | Base URL of the vault service, e.g. `http://vault:8200` |
| `--hashi-vault-token` | Authentication token for accessing HashiCorp Vault |
| <nobr>`--hashi-vault-key-path`</nobr> | Key path(s) in the K/V secret engine where validator signing keys are stored. Can be repeated for multiple paths |
| <nobr>`--hashi-vault-key-prefix`</nobr> | Key prefix(es) in the K/V secret engine. Can be repeated for multiple prefixes |
| <nobr>`--hashi-vault-parallelism`</nobr> | Number of parallel requests to K/V secrets engine |

:::custom-warning[Secrets Engine Must Be Mounted at `secret`]
Keys are read from `/v1/secret/data/<key-path>`, and the engine name is not configurable in Operator Service v4 — the `--hashi-vault-engine-name` option available in v3 was removed. If your K/V engine is mounted somewhere other than `secret`, mount a copy at `secret` for the Operator Service to use.
:::

For additional configuration, see the full list of [optional flags](../start-operator#optional-flags).
