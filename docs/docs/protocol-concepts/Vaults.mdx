...
><span style={{color: '#1f2937', fontWeight: '500'}}> Note: Exit signatures can become invalid if the oracle network changes (e.g., DAO rotates oracles after a key compromise). The operator service periodically checks for outdated signatures and updates them on-chain when needed.</span>
...
Vaults participate in MEV redistribution through [SharedMevEscrow](https://etherscan.io/address/0x48319f97e5da1233c21c48b80097c0fb7a20ff86#code) contract, providing consistent returns across participating pools regardless of individual block proposal frequency.

**MEV Escrow** - Keep What You Earn

Vaults collect MEV rewards through individual OwnMevEscrow contract, preserving full value capture but creating more volatile returns based on actual validator block proposals.

This choice is permanent – vault operators must decide during creation and cannot change MEV strategies later.
...
><span style={{color: '#1f2937', fontWeight: '500'}}> Note: StakeWise always verifies the validity of deposit data submitted to the vault, ensuring it contains the correct withdrawal address and has not been used before.</span>
