...
For detailed information about the reward distribution process, see [Oracles →](./Oracles).
:::
...
You can also use [manual exit](#exit-validators), although this command should only be used in extreme scenarios.
...
2. **Initiate exits if needed**. If available ETH is insufficient, the Operator Service initiates validator exits to free up the remaining amount.
3. **Oracle enforcement of exits**. If the Operator Service does not initiate partial or full withdrawals, the Oracle network will automatically execute full withdrawal after 24 hours.

The exit queue advances whenever the Vault **syncs state** — both on **user actions**
(entering the queue, deposits/withdrawals) and on the **operator Service-driven syncs** (e.g., harvest).
Throughout the process, user assets are safe in the Vault's smart contracts and are never controlled by any third party.

Exit processing time depends on the Ethereum network congditions.
