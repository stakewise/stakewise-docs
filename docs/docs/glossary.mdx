...
### DAO-Approved Vault
Vaults that meet strict DAO-defined criteria and qualify for enhanced LTV ratios (up to 99.99% for osETH, 99.95% for osGNO). Requirements include minimum stake thresholds, fee caps, above-median performance, latest Vault version, and substantial SWISE bond collateral.
...
### Keeper
The StakeWise smart contract that coordinates validator registration approvals between Operator Services and the Oracle network. The Keeper ensures that validator registrations receive proper consensus from Oracles before being finalized.
...
### Meta Vault
A Vault type that does not register validators directly, instead delegating accumulated assets to sub-Vaults managed by the Vault Admin. Deposits are distributed across underlying sub-Vaults (up to 50 maximum) according to curator-defined allocation logic. Used for diversified staking strategies.
...
### Redemption
In 90% LTV Vaults, a mechanism that activates when an osToken position's LTV reaches 91.5%. Anyone can burn part of that position's osTokens in exchange for an equivalent share of the Vault's collateral, restoring the position's LTV to 90%. The staker whose tokens are redeemed keeps the osTokens burned on their behalf, losing no value.
...
### Unboost Queue
A queue system that manages withdrawal requests from Boost positions. When users request to unboost, they enter the queue and can claim their assets as they become available from Aave.
...
### Own Escrow
An MEV strategy where block proposal rewards are collected solely by individual Vaults. Own Escrow allows a Vault to choose an independent approach to earning block production rewards, like using any relay of choice, because it does not rely on sharing such rewards with other Vaults. Own Escrow targets maximum value capture but with increased variability.

### Vault Fee
A percentage fee charged by Vault operators on staking rewards, ranging from 0% to 100%. The fee is automatically deducted when Vault state is updated and transferred to the designated fee recipient. Fees can only be increased by 20% at a time with a 3-day delay between updates.
...
### Vault Shares
Non-transferable tokens issued by Standard Vaults representing a user's proportional stake in the Vault. Shares accrue rewards over time and can be used as collateral for minting osTokens.
