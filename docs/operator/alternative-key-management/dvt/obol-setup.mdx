...
- `--cluster-dir`: Parent directory containing a number of .charon subdirectories from the required threshold of nodes in the cluster. (default "./")
- `--withdrawal-addresses`: Comma separated list of Ethereum addresses to receive the returned stake and accrued rewards for each validator. Either provide a single withdrawal address or withdrawal addresses for each validator.
- `--fee-recipient-addresses`: Comma separated list of Ethereum addresses of the fee recipient for each validator. Either provide a single fee recipient address or fee recipient addresses for each validator.
- `--nodes 3`: The number of charon nodes in the cluster. Minimum is 3.
- `--network`: Ethereum network to create validators for. Options: mainnet, goerli, sepolia, hoodi, holesky, gnosis, chiado. (default "mainnet").
- `--split-existing-keys`: Split an existing validator's private key into a set of distributed validator private key shares.
- `--split-keys-dir`: Directory containing keys to split. Expects keys in `keystore-*.json` and passwords in `keystore-*.txt`. Requires `--split-existing-keys`.
:::
...
The `docker-compose.yml` file below runs the complete DVT stack:
...
- **3 Charon clients** (node0, node1, node2) - coordinate distributed validator duties
- **3 Teku validator clients** - sign attestations and proposals
- **Geth** - execution layer client
- **Lighthouse** - consensus layer client
- **MEV-boost** - for block building

:::custom-warning[Fee Recipient Address]
In the `docker-compose.yml` file, replace `[ENTER YOUR VAULT FEE RECIPIENT ADDRESS HERE]` with your Block reward recipient address. You can find it your Vault's Details section.
:::
...
1. Copy the `docker-compose.yml` file above to your server
2. Place it in the same directory as your `.charon` folder
3. Update the fee recipient address in the file
4. Run the stack:
