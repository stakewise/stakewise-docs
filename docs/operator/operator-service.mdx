...
The Operator Service supports registering both `0x01` and `0x02` validators within the same Vault using the `--validator-type` option to specify the validator type. By default, the operator registers new `0x02` validators, prioritizing funding of existing compound validators when possible; otherwise, it registers new validators up to the maximum balance via the `register_new_validators` function.
...
The Operator Service automatically processes partial ETH/GNO withdrawals from validators. The system prioritizes withdrawing all available ETH/GNO through partial withdrawals first, followed by full validator withdrawals when necessary.
...
<Admonition type="info" icon={<img src={useBaseUrl('/img/eth_coin.png')} alt="Warning" style={{ width: 20, height: 20 }}/>} title="Withdrawal Processing">
  Every 24 hours, the Operator Service checks the current exit queue. If the requested amount can be processed via partial withdrawals, it submits them through the execution layer
</Admonition>
