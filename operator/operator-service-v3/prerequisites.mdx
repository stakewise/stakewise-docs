...
See [Where to Start: Create Your Vault →](/operator/where-to-start-create-your-vault) on how to create a Vault.
...
A new experimental feature allows the Operator Service to automatically install and manage execution and consensus clients (Reth and Lighthouse). See [Automated Node Setup →](/operator/manage-validators/automated-node-setup) for details.
:::
...
If your Vault uses the Smoothing Pool, you must use one of the StakeWise DAO-approved MEV relays (see [Smoothing Pool Relays →](/operator/smoothing-pool-relays)).
