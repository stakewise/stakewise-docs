...
You should now have multiple folders within `./.charon/cluster/`, one for each node created.
...
After initial setup, run 3 Charon clients and 3 validator clients. You can run them on a single server or distribute across multiple servers.

Copy the `docker-compose.yml` file below to the server in the same directory with `.charon` and run:

```bash
docker compose up -d
```

:::custom-notes[Fee Recipient Address]
