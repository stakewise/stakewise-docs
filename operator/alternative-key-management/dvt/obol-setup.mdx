...
Key shares can be produced two ways. Splitting an existing validator key is simpler, but the full key has to exist on one machine first. A Distributed Key Generation (DKG) ceremony avoids that: rather than one party generating the full private key and splitting it among the nodes, every operator's Charon client takes part so the shares are generated together, and the full private key is never constructed on any machine.
...
:::custom-warning[Keystore Password Configuration]
If you intend to split existing validator keys (the [Create a DV Alone](#create-a-dv-alone) flow), generate them with the `--per-keystore-password` flag:

```bash
./operator create-keys --per-keystore-password
```

Charon expects a `keystore-*.txt` password file next to every `keystore-*.json` in `--split-keys-dir`. Without this flag `create-keys` writes a single shared `password.txt`, and the split fails.
:::
...
- **Vault contract address** — in the Vault page URL, or its Details section.
- **Block reward recipient** — the "Block reward recipient" field in your Vault page settings.
:::
...
| <nobr>`--nodes`</nobr> | Number of Charon nodes in the cluster. Charon's minimum is 3; the examples here use 4, the smallest cluster whose consensus tolerates one faulty node |
...
3. On that machine, copy the sample env file for your network (`.env.sample.mainnet` or `.env.sample.hoodi`) to `.env`, adjust it if needed, and run `docker compose up -d`.
...
:::custom-warning[Do Not Submit the Deposit Data]
Obol's guide finishes by activating the validators with the generated `deposit-data.json`. Do not do that here. The Vault holds the stake and makes the deposit itself, with withdrawal credentials derived from the Vault address, so the ceremony's deposit data goes unused. Activation happens through the registration flow in the steps below.
:::
...
The DVT Sidecar supports Ethereum mainnet and Hoodi. Gnosis is not supported for this flow, even though Charon can create Gnosis clusters.
:::
...
2. In a `data/` directory next to the `.env`, place this node's `validator_keys/` directory and its `cluster-lock.json`.

3. Set the values for your Obol node. The container runs from `/app`, so every path must point inside the mounted `/data`:
...
OBOL_KEYSTORES_DIR=/data/validator_keys
# The cluster-lock.json from the DKG ceremony
OBOL_CLUSTER_LOCK_FILE=/data/cluster-lock.json
...
The `EXECUTION_ENDPOINT` and `CONSENSUS_ENDPOINT` entries in `.env.example` are only read when `CLUSTER_TYPE=SSV`, so an Obol node can leave them unset.
...
2. In a `data/` directory, place the files the Relayer reads:
   ...
   - `public_keys.txt`: the cluster's distributed validator public keys, one per line. Extract them from the `cluster-lock.json` produced by the ceremony:

     ```bash
     jq -r '.distributed_validators[].distributed_public_key' cluster-lock.json > data/public_keys.txt
     ```
...
The Operator Service has no keystores here, so it runs in [Relayer (API Mode)](/operator/alternative-key-management/api-mode) pointing at your DVT Relayer.
...
If you run the nodes with Obol's CDVN setup (the solo production tab above), it ships a local Grafana dashboard. Check your nodes' health at `http://localhost:3000/d/charon_overview/`. For hosted dashboards and alerting in any setup, see [Obol monitoring ↗](https://docs.obol.org/run-a-dv/start/obol-monitoring).
