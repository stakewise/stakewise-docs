...
- **[Remote Signer ↗](./alternative-key-creation/remote-signer)** - Query a remote signer to get signatures for validator deposit and exit messages
- **[HashiCorp Vault ↗](./alternative-key-creation/hashicorp-vault)** - Load signing keys from remote HashiCorp Vault instance
- **[API Mode ↗](./alternative-key-creation/api-mode)** - Run operator as API service with external key management via relayer

### Add validator keys to Vault

You can always add more validator keys to your Vault.
For that, you need to generate new validator keys as described in [Step 2: Create Validator Keys →](#step-2-create-validator-keys).
...
<Admonition type="info" icon={<img src={useBaseUrl('/img/Email.png')} alt="Key Upload" style={{ width: 20, height: 20 }}/>}  title="Key Upload Process">
  <ol style={{ listStyleType: "none", paddingLeft: "0" }}>
...
Enter the network name: holesky
