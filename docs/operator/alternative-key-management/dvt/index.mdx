...
Validators have two public-private key pairs: active keys for consensus participation (block proposals, attestations) and withdrawal credentials that determine where staked funds can be sent upon exit. While withdrawal credentials can remain offline, validator keys must stay online continuously, making them vulnerable to compromise.
...
StakeWise Vaults support both [Obol ↗](https://docs.obol.org/learn/readme/key-concepts) and [SSV ↗](https://docs.ssv.network/) DVT technologies, offering an additional layer of security and decentralization. This gives operators flexibility in how they secure their validators.
