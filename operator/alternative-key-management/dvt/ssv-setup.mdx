...
The logs let you verify startup. Press Ctrl-C to detach — `--restart unless-stopped` brings the node back up on its own. To start detached in the first place, add `-d` after `docker run`.
:::
...
Your fee (in SSV tokens) is charged per validator that selects you, and earnings accumulate in your operator balance for you to withdraw at any time. You can change it later: decreases apply immediately, increases go through a waiting period that [protects stakers ↗](https://docs.ssv.network/operators/operator-management/updating-operator-fees). If you only run your own Vault's validators, the fee is money you pay yourself.
:::
