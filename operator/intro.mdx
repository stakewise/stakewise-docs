...
:::custom-tips[Experimental Feature Available]
Many users confuse the Operator Service with a complete staking setup. This causes difficulties during configuration, as execution and consensus clients must be installed and run manually.
To simplify this process, we added a new experimental feature that allows the Operator Service to handle these steps automatically. See [Automated Node Setup →](node-setup).
:::
