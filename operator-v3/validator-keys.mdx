...
`config.json` - Configuration file for your Vault.

`deposit_data.json` - Contains validator registration information including public key, withdrawal credentials, and cryptographic signature. This file must be uploaded to your Vault (Step 5) to authorize the Operator Service to register your validators.

`keystores/` directory containing:
  - Validator keystores following [EIP-2334 ↗](https://eips.ethereum.org/EIPS/eip-2334) for deterministic key generation.
  - `password.txt` - Password for all generated keystores.
...
:::custom-warning[Fund Your Hot Wallet]
Send some ETH (or xDAI for Gnosis) to this wallet for gas expenses. Each validator registration costs ~0.01 ETH at 30 Gwei gas price.
:::
