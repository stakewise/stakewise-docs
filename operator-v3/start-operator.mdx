...
description: Start the StakeWise Operator v3 Service with the required flags and optional configuration for validator registration and Vault management.
sidebar_position: 5
---
...
- `--keystores-dir` - The directory with validator keys in the [EIP-2335 ↗](https://eips.ethereum.org/EIPS/eip-2335) standard. The folder must contain either a single `password.txt` password file  for all keystores or separate password files for each keystore with the same name as keystore, but ending with `.txt` (e.g., `keystore1.json`, `keystore1.txt`)
...
Operator Service can be configured via environment variables instead of CLI flags. Copy [this example file ↗](https://github.com/stakewise/v3-operator/blob/master/.env.example) and save it as `.env`. Adjust as necessary based on your node configuration.
