...
Keystores are saved to `~/.stakewise/[vault address]/keystores`. By default, a single `password.txt` file contains the password for all generated keystores. Use the `--per-keystore-password` flag to generate individual password files instead (e.g., `keystore-001.txt`, `keystore-002.txt`).
