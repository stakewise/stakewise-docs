...
The Operator Service is indispensable in order to successfully register validators on the platform and manage ongoing validator operations, including exit signature rotation and Vault state updates.
...
Validator exits are handled by StakeWise Oracles. You can manually trigger validator exits if needed.
...
Keeps your Vault synced by calling the `updateState` function of the Vault contract. This can be configured to run automatically every 12 hours by passing the `--harvest-vault` flag to the start command, simplifying contract interactions and reducing gas costs for your users. By default, the Vault state sync is included in the transaction whenever the user interacts with the Vault (deposits, withdrawals, etc.).

### Automated Withdrawals (Reward Splitter)

You can enable periodic withdrawal of rewards for the vault's fee shareholders by passing the `--claim-fee-splitter` flag to the start command. This requires setting the operator wallet as the Fee Claimer in the vault settings.
