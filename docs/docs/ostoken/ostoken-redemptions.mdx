...
The holder sends osToken to the `OsTokenRedeemer` contract and gets a ticket — a unique cumulative index recording their place in [the queue](#queue) and how much they're owed. The user has now committed and holds the right to claim ETH/GNO later.

### 2. Operator Service Prepares the Next Redemption

The Operator Service monitors the on-chain state. If a previous redemption round has redeemed ETH/GNO waiting to be [checkpointed](#checkpoints), it first calls `processExitQueue` to finalize that batch so holders can claim it.
